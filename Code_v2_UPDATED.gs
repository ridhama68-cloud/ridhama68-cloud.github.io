...
function doGet(e) {
  ...
  try {
    ensureSetup_();
    // Writes may arrive via GET with a ?payload=<json> param. This dodges the
    // cross-origin POST/CORS breakage Apps Script sometimes has after a redeploy
    // (GET's redirect keeps its CORS header; POST's redirect can lose it).
...
// Shared write handler for POST and GET(?payload=). Serializes writes with a lock.
